...
\begin{document}
    ...
    Per l'obiettivo, sono stati utilizzati dei grafi presi dal dataset~\cite{dataset}, come ad esempio babyface, BVZ-tsukuba e K2Z-venus.
    ...
    Definendo $C$ la capacità massima tra gli archi del grafo, la complessità temporale dell'algoritmo è $O(m^{2}\log C)$.
    ...
    \begin{figure}[ht]
        \centering
        \input{../internal/analysis/scalability_high}
        \caption{Scalabilità}
        \label{fig:scal}
    \end{figure}
    ...
    \begin{figure}[ht]
        \centering
        \input{../internal/analysis/capacity}
        \caption{}
        \label{fig:}
    \end{figure}

    TODO: analisi su questo grafico

    \section{Analisi su dataset reali}\label{sec:analisi_reale}

    \begin{figure}[ht]
        \centering
        \input{../internal/analysis/real}
        \caption{Prestazioni sul dataset di Visione Artificiale BVZ-Tsukuba}
        \label{fig:real_test}
    \end{figure}

    Dal grafico in Figura~\ref{fig:real_test}, si può notare come negli algoritmi di visione artificiale, come BVZ-Tsukuba~\cite{dataset}, a dominare sia l'algoritmo di Dinic, nella versione ricorsiva.
    Shortest Augmenting Path, sia da solo sia nella versione con Capacity Scaling, ha delle prestazioni leggermente inferiori.

    L'algoritmo più in difficoltà risulta essere nuovamente Capacity Scaling, che risolve il problema in più di $100s$ (per confronto, Dinic in versione ricorsiva ha risolto il problema in $\approx 0.1s$).
    L'enorme differenza tra i due algoritmi potrebbe essere data dalla tipologia di dataset utilizzato: BVZ-Tsukuba utilizza capacità relativamente piccole, con $\log C = 10$, iterando dunque su poche fasi ma molto complesse.

    TODO: inserire performance su Venus e Babyface
